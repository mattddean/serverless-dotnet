...
module.exports = {
  pack() {
    ...
    this.serverless.cli.log('servicePath: ', servicePath)
    this.serverless.cli.log('hasPackage: ', this.options.hasOwnProperty('package'))
    this.serverless.cli.log('this.options.package: ', this.options.package)

    const artifactDirectoryPath = this.options.hasOwnProperty('package') ?
      this.options.package : 
      path.join(servicePath, '.serverless');

    this.serverless.cli.log('artifactFilePath: ', artifactFilePath)
    this.serverless.cli.log('zipFileName: ', zipFileName)
